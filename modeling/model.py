...
class BertLMPredictionHead(nn.Module):
    def __init__(self, la_dim, token_dim, vocab_size):
...
class BertForMaskedLM(nn.Module):
    ...
    def __init__(self, vocab_size, config):
        ...
        if self.has_FC and self.has_la_emb:
            cat_dim = config.token_dim + config.la_dim
            self.transform = BertPredictionHeadTransform(cat_dim, config.hiddent_act)
        elif self.has_FC and not self.has_la_emb:
            self.transform = BertPredictionHeadTransform(config.token_dim, config.hiddent_act)
        if self.has_la_emb:
            self.la_tag_linear = nn.Embedding(config.la_num, config.la_dim)
    ...
    def forward(self, input_ids, input_len, tgt_la):
        ...
        if self.has_FC and self.has_la_emb:
            ...
            if self.sentloss_before_fc:
                ...
                raise ValueError
        elif not self.has_FC and self.has_la_emb:
            prediction_scores = self.decoder(cat_la_output)
            return prediction_scores, sent_emb
        elif self.has_FC and not self.has_la_emb:
            transformed_output = self.transform(sent_emb)
            prediction_scores = self.decoder(transformed_output)
            if self.sentloss_before_fc:
                return prediction_scores, sent_emb
            else:
...
class BilingualModel(nn.Module):
    ...
    # @amp.autocast()
    def forward(self, input_ids1, input_len1, input_ids2, input_len2, masked_lm_label, masked_la, tgt_la1, tgt_la2):
        ...
        if self.has_sentence_similarity_loss: 
            ...
            sent_sim_loss_2 = torch.sum(torch.log(torch.cos((mono_products_1_softmax - mono_products_2_softmax) * math.pi)) * (-1)) / ((sent_emb1.size()[0] - 1) ** 2)
